...
export DEPLOY_SERVICE=openape-plans.service
export DEPLOY_PORT=3004
export PUBLIC_HOST=plans.openape.ai

curl -fsSL https://raw.githubusercontent.com/openape-ai/plans/main/scripts/server-setup.sh | bash
...
- Fill in `/home/openape/projects/openape-plans/shared/.env` with 32-char
  secrets (`openssl rand -hex 32` is a good default).
- Attach TLS: `sudo certbot --nginx -d plans.openape.ai`.
...
- `DEPLOY_PUBLIC_URL` — full https URL for the health check (default
  `https://plans.openape.ai`)

Optional overrides (with defaults shown):

- `DEPLOY_USER=openape`
- `DEPLOY_BASE=/home/openape/projects/openape-plans`
- `DEPLOY_PORT=3004`
- `DEPLOY_SERVICE=openape-plans.service`
...
- `systemctl status openape-plans.service` shows `active (running)`.
- `curl -fsS https://plans.openape.ai/api/me` returns `401` (healthy).
...
```bash
ssh openape@<host>
ls -1 ~/projects/openape-plans/releases/
ln -sfn ~/projects/openape-plans/releases/<older-ts> ~/projects/openape-plans/current
sudo systemctl restart openape-plans.service
```

## 6. Observability

- `sudo journalctl -u openape-plans.service -f` — live logs.
- `curl -fsS https://plans.openape.ai/api/me` — 401 means healthy, anything
  else is a signal.
