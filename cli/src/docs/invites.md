...
```
ape-plans teams invite <team-id> [--max-uses N] [--expires-in DUR] [--note "..."]
```
...
stderr. Suitable for `INVITE_URL=$(ape-plans teams invite <id>)`.

## Accept

```
ape-plans accept <url-or-token>
```

Accepts both the full URL (`https://plans.openape.ai/invite?t=...`) and the
...
ape-plans teams revoke-invite <invite-id>      # revoke (JWT becomes invalid)
```
