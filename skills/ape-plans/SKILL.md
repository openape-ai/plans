...
2. **Mark progress as you go** using `ape-plans edit --replace-section "## Progress"` — check off milestones, add timestamps.
3. **Append discoveries** as you find them — via `ape-plans edit --append-body` or a dedicated `## Surprises & Discoveries` section. Non-obvious facts. Evidence (actual error messages, curl output, test names).
...
```
ape-plans show <id>                                     # read it back
ape-plans edit <id> --append-body --body-from-stdin     # add a new block
ape-plans edit <id> --replace-section "## Progress" --body-from-stdin   # tick off
ape-plans status <id> active                            # mark in-flight
ape-plans status <id> done                              # wrap up
```
...
- **Every command is literally runnable** — `pnpm --filter @openape-plans/app build`, not "run the build".
...
If you're an agent, use the existing Patrick-template style as reference — see `~/PLANS-TEMPLATE.md` for a long-form example, and look at existing plans on plans.openape.ai for real examples to imitate.

## When to use — decision table

| Situation | Use this skill? | Notes |
|-----------|:---------------:|-------|
| User asks for a plan for anything non-trivial | **Yes** | Default — propose it and create the plan on plans.openape.ai |
...
| Another agent will continue | **Yes** | Invite them via `ape-plans teams invite` |
...
ape-plans teams                          # `*` marks your default team
ape-plans teams use <team-id>            # set a default so you don't repeat --team
ape-plans list --json                    # all plans you can see
ape-plans list --status active           # what's in flight

# Read before writing — never overwrite someone else's work silently
ape-plans show <plan-id>                 # Markdown body to stdout
ape-plans show <plan-id> --json          # full object incl updated_by

# Create
ape-plans new --title "..." --body-from-stdin <<'EOF'
# Goal
...
EOF
...
ape-plans edit <id> --append-body --body-from-stdin <<<"
## Surprises
- Discovered X; evidence: <paste actual output>"
ape-plans edit <id> --replace-section "## Progress" --body-from-stdin <<'EOF'
- [x] M1 — done 2026-04-22
- [ ] M2 — next
EOF
ape-plans status <id> active
ape-plans status <id> done

# Share
ape-plans teams invite <team-id> --max-uses 1 --expires-in 24h
ape-plans accept <url-or-token>          # on the receiving side

# Browse (humans)
ape-plans open <id>                      # opens the web view

# Full reference
ape-plans docs agent
ape-plans <cmd> --help
```

## Common pitfalls to avoid

- **Silent overwrite.** `ape-plans edit --body-from-file` replaces the whole body. If someone else edited since you last read, their work is gone. Prefer `--append-body` / `--replace-section` for incremental updates, or `show --json` first to verify `updated_at`/`updated_by`.
...
- `ape-plans docs agent` — static Markdown reference (JSON schemas, error codes). This skill tells you *when* to use the CLI; the docs tell you *how* each command behaves.
- `ape-plans --help` — per-command flags + one example. Read when you need the exact syntax.
- `~/PLANS-TEMPLATE.md` — long-form writing template. Use as starting point for non-trivial plans.
- `.claude/plans/<name>.md` — local throwaway. Not cross-session, not searchable by others.

Start by reading the plans that already exist on the server — `ape-plans list --json`. If one matches the task at hand, continue it. Don't fork silently.
