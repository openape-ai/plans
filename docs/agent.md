...
write, and share plans through `plans.openape.ai`. Humans can do the same
...
ape-plans login you@example.com
```
...
```
ape-plans show 01HXX... --json | jq 'del(.body_md)'   # metadata
ape-plans show 01HXX...                               # just the Markdown
```
...
echo '# My plan' | ape-plans new --team 01H... --title "Auth rewrite" --body-from-stdin --json

# Update
ape-plans edit 01H... --body-from-file ./updated.md
ape-plans status 01H... done
```
...
```
ape-plans teams invite 01H... --max-uses 1 --expires-in 24h --note "agent handoff"
...
```
ape-plans accept https://plans.openape.ai/invite?t=eyJhbGc...
```

They are now an editor on the same team and can read/write the same plans.

## When to use plans.openape.ai vs. local files

| Situation | Use |
|-----------|-----|
| Plan must survive this session | plans.openape.ai |
| Another agent or human must read it | plans.openape.ai |
| Runs from different devices | plans.openape.ai |
...
|------|---------|--------------|
| 401  | Token missing/expired | Prompt user to `ape-plans login` |
...
- `ape-plans docs invites` — invite flow deep dive
- `ape-plans docs auth` — auth internals and rotation
- `ape-plans docs errors` — full error code reference
- `ape-plans docs cli` — complete command reference
