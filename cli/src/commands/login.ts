...
/**
 * Log in to plans.openape.ai.
 *
 * Paste-based because plans.openape.ai relies on DDISA-discovered identity
 ...
 *   3. Piped stdin (`echo $TOKEN | ape-plans login <email>`).
 *
 * EXAMPLES
 *   $ ape-plans login patrick@example.com
 *   Open in browser: https://plans.openape.ai/cli-login
 *   After signing in, paste the token shown there.
 *   Token: eyJhbGciOi…
 *   Logged in as patrick@example.com (endpoint: https://plans.openape.ai)
 *
 *   $ ape-plans login patrick@example.com --token eyJhbGciOi...
 *   $ echo "$OPENAPE_CLI_TOKEN" | ape-plans login patrick@example.com
 *
 * Token goes to ~/.openape/auth-plans.json (chmod 600). 30-day lifetime;
 * run `ape-plans login` again to refresh.
 */
export const loginCommand = defineCommand({
  ...
  args: {
    ...
    endpoint: {
      type: 'string',
      description: 'Override plans endpoint (default https://plans.openape.ai or APE_PLANS_ENDPOINT).',
    },
  },
  async run({ args }) {
